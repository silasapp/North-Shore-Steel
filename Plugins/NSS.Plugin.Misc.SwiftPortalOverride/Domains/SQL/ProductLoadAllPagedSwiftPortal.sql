...
CREATE PROCEDURE [dbo].[ProductLoadAllPagedSwiftPortal]
(
	@ShapeIds			nvarchar(MAX) = null,	--a list of shape IDs (comma-separated list). e.g. 1,2,3
...
BEGIN
	...
	BEGIN
		...
		SET @sql = @sql + '
			...
			WHERE sh.[Name] = @OriginalKeywords OR psh.[Name] = @OriginalKeywords '
	...
	END
		...
		--filter by shape IDs
	SET @ShapeIds = isnull(@ShapeIds, '')	
	CREATE TABLE #FilteredShapeIds
	(
		ShapeId int not null
	)
	INSERT INTO #FilteredShapeIds (ShapeId)
	SELECT CAST(data as int) FROM [nop_splitstring_to_table](@ShapeIds, ',')	
	DECLARE @shapeIdsCount int	
	SET @shapeIdsCount = (SELECT COUNT(1) FROM #FilteredShapeIds)
