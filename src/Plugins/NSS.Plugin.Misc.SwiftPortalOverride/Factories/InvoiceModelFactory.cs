...
namespace NSS.Plugin.Misc.SwiftPortalOverride.Factories
{
    public class InvoiceModelFactory : IInvoiceModelFactory
    {
        ...
        public async Task<CompanyInvoiceListModel> PrepareInvoiceListModelAsync(int companyId, CompanyInvoiceListModel.SearchFilter filter)
        {
            // search nss api
            ...
            if (filter.IsClosed)
                (_, response) = await _apiService.SearchClosedInvoicesAsync(companyId, request);
            else
                (_, response) = await _apiService.SearchOpenInvoicesAsync(companyId, request);
