...
namespace NSS.Plugin.Misc.SwiftApi.Controllers
{
    public class ProductMTRController : BaseApiController
    {
        ...
        public async Task<IActionResult> CreateProductMTR(
            ...
            )
        {
            ...
            var attr = await _genericAttributeService.GetAttributeByKeyValueAsync(Constants.ItemIdFieldAttribute, id.ToString(), nameof(Product));
            if (attr == null)
                product = null;
            else
            ...
            await _genericAttributeService.SaveAttributeAsync(product, Constants.MTRFieldAttribute, blobUri);
        ...
        [ProducesResponseType(typeof(string), (int)HttpStatusCode.Unauthorized)]
        [GetRequestsErrorInterceptorActionFilter]
        public async Task<IActionResult> DeleteProductMTR(
            int id
            )
        {
            if (!ModelState.IsValid)
            ...
            await CustomerActivityService.InsertActivityAsync("APIService", "Starting Product MTR Delete", null);
            ...
            // delete attr
            await _genericAttributeService.SaveAttributeAsync<string>(product, Constants.MTRFieldAttribute, null);
