...
namespace NSS.Plugin.Misc.SwiftApi.Controllers
{
    public class OrderFundsCaptureController : BaseApiController
    {
        ...
        public async Task<IActionResult> CaptureOrder(int orderId)
        {
            ...
            await _logger.InsertLogAsync(Nop.Core.Domain.Logging.LogLevel.Debug, $"Swift API - CaptureOrder - orderId = {orderId}");

            // get entity order
            var sysOrderId = (await _genericAttributeService.GetAttributeByKeyValueAsync(SwiftCore.Helpers.Constants.ErpOrderNoAttribute, orderId.ToString(), nameof(Order)))?.EntityId;

            if (sysOrderId == null)
                return Error(HttpStatusCode.BadRequest, "order", "not found.");

            // get authID
            var order = await _orderService.GetOrderByIdAsync(sysOrderId.Value);
            ...
            var (capture, error) = await _payPalProcessor.CaptureAuthorizationAsync(_swiftCoreSettings, order.AuthorizationTransactionId);
